        ...
        for( unsigned int candIndex = 0; candIndex < diPhotons->size() ; candIndex++ ) {
            ...
            for( size_t ijet=0; ijet < cleaned_jets.size();++ijet){
                ...
                if (ctagCvsL > ctagCvsL_ref) {
                  ...
                  //  ctagCvsB_ref = ctagCvsB;
                    jet1 = jet_1;
                }
            }
           // if (ctagCvsL<0.099) continue;//FIXME to customize. For now applying medium WP. 
          //  if (ctagCvsB<0.325) continue;//FIXME to customize. For now applying medium WP. 
            auto & leadJet = jet1; 
